...
function Recipe({
  ...
  fullStCount: number;
  fittedOrSlouchy: "fitted" | "slouchy";
  height: number;
  rowsPerInch: number;
}) {
  useEffect(() => {
  ...
  const crownSetup = fullStCount % 8;
      ...
      <div className="row">
        ...
        <div className="col-lg-8">
          ...
          <p>
            ...
            {fullStCount} sts. Join to work in the round, being careful not to
            twist.
          </p>
          <p>
            Rnd 1: *k2, p2; rep from * to end. Work rnd 1 until work measures 2
            inches from CO edge, or desired height of brim.
          </p>
          <p>
          ...
          {fittedOrSlouchy === "fitted" && (
            <p>
              ...
              {height < 8
                ? Math.round((Number(height) - 10 / rowsPerInch) * 10) / 10
                : Math.round((Number(height) - 16 / rowsPerInch) * 10) /
                  10}{" "}
              inches from CO edge.*
            </p>
          )}
          {fittedOrSlouchy === "slouchy" && (
            <p>
              Work in Stockinette stitch until work measures about&nbsp;
              {height < 8
                ? Math.round((Number(height) + 2 - 10 / rowsPerInch) * 10) / 10
                : Math.round((Number(height) + 2 - 16 / rowsPerInch) * 10) /
                  10}{" "}
              inches from CO edge.*
            </p>
          )}
          <p>
            *If you plan to wear your hat with a folded brim, fold your brim and
            measure from the bottom folded edge instead of the CO edge.
          </p>
          {crownSetup > 0 && (
            <div>
              <h3>Crown Setup</h3>
          ...
          <h3>Finishing</h3>
          <p>
            Cut yarn, leaving a 6-inch tail. Draw tail through remaining sts and
            secure. Weave in ends, block, and enjoy!
          </p>
        </div>
      </div>
    </div>
